 ...
 * Validasi dengan Supabase Auth untuk memastikan session masih valid (hanya jika online)
 * Check expiration timestamp
 * Jika offline, langsung return user dari localStorage tanpa validasi ke Supabase
 */
export async function getCurrentUser() {
    try {
        ...
        // Jika offline, langsung return user dari localStorage tanpa validasi ke Supabase
        // Ini memastikan user bisa login saat offline tanpa perlu validasi ke server
        if (!isOnline()) {
            logger.log('Device is offline, using cached user session without Supabase validation');
            return user;
        }
        
        // Validasi session dengan Supabase Auth (hanya jika online dan supabase sudah diinit)
        // Pastikan tidak memanggil validasi jika offline (double check untuk safety)
        if (supabase && isOnline()) {
            try {
                const { data: { session }, error } = await supabase.auth.getSession();
                if (error || !session) {
                    // Session tidak valid atau expired
                    // Double check: jika tiba-tiba offline saat validasi, tetap return user
                    if (!isOnline()) {
                        logger.warn('Supabase session invalid but device went offline during validation, using cached session');
                        return user;
                    }
                    // Online tapi session tidak valid, hapus dari localStorage
                    clearExpiredSession();
                    return null;
                }
                ...
                // Jika ada error saat validasi, cek apakah offline
                // Bisa terjadi jika koneksi terputus saat validasi
                if (!isOnline()) {
                    logger.warn('Supabase validation error but device is offline, using cached session:', authError);
                    return user;
                }
                // Online tapi ada error, hapus session
        ...
        // Jika supabase belum diinit atau offline, return user dari localStorage (untuk fallback)
        return user;
    } catch (error) {
        logger.error('Get current user error:', error);
        // Jika error parsing, hapus session yang mungkin corrupt
        clearExpiredSession();
        return null;
    }
}
