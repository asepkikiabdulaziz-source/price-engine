...
export function calculateTotal({
    ...
    // Filter invoice discounts by availability (store type, zone, region, depo)
    // Same as other promo types (principal, group, bundle, free product)
    const availableInvoiceDiscounts = (invoiceDiscounts || []).filter(discount => {
        const isAvailable = isPromoAvailable(
            discount.promo_id,
            'invoice',
            promoAvailabilityRules,
        ...
        // Debug logging untuk invoice discount filtering
        if (window.DEBUG_INVOICE_DISCOUNT) {
            console.log(`Invoice discount ${discount.promo_id}: ${isAvailable ? 'AVAILABLE' : 'NOT AVAILABLE'}`, {
                storeType,
                userZona,
                userRegion,
                userDepo,
                rules: promoAvailabilityRules.filter(r => r.promo_id === discount.promo_id && r.promo_type === 'invoice')
            });
        }
    ...
    // Calculate invoice discount (hanya dengan discount yang available)
    result.invoiceDiscount = calculateInvoiceDiscount(
        ...
        availableInvoiceDiscounts, // Filtered by availability
        paymentMethod || 'COD'
    );
